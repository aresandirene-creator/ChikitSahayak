    ...
    return (
      <div className="max-w-3xl mx-auto">
        <div className="text-center py-20">
          ...
          <h3 className="text-xl font-semibold text-emerald-900">{t("summaryGenerating")}</h3>
    ...
    return (
      <div className="max-w-3xl mx-auto space-y-6">
        <div>
          ...
          <h2 className="mt-3 text-2xl sm:text-3xl font-bold text-emerald-900">{t("summaryTitle")}</h2>
        </div>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      {showMarkdown ? (
        ...
        <Card className="border-emerald-100 shadow-sm">
          <CardHeader>
            <CardTitle className="text-sm font-semibold text-emerald-900 flex items-center gap-2">
              <FileText className="size-4 text-emerald-600" /> {t("summaryViewMarkdown")}
            </CardTitle>
      ...
      <Card className="border-emerald-100 shadow-sm">
        <CardHeader>
          <CardTitle className="text-sm font-semibold text-emerald-900 flex items-center gap-2">
            <Pencil className="size-4 text-emerald-600" /> {t("summaryNotesTitle")}
          </CardTitle>
      ...
      <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-4 flex items-start gap-3 text-sm text-emerald-800">
        ...
        <div>
          ...
          <p className="text-emerald-700/80 mt-1">{t("summaryDisclaimerBody")}</p>
        </div>
      </div>
...
function StatusBadge({ status, t }: { status: string; t: (k: never) => string }) {
  if (status === "confirmed") {
    return <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200"><CheckCircle2 className="size-3 mr-1" /> {t("summaryStatusConfirmed")}</Badge>;
  }
  if (status === "rejected") {
  ...
  return <Badge className="bg-emerald-50 text-emerald-700 border-emerald-200"><Sparkles className="size-3 mr-1" /> {t("summaryStatusDraft")}</Badge>;
}
