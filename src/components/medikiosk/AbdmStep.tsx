...
export function AbdmStep() {
  ...
  const ACTIONS = [
    { key: "link_abha", icon: Fingerprint, title: t("abdmActionLinkAbhaTitle"), desc: t("abdmActionLinkAbhaDesc"), color: "bg-red-100 text-red-700" },
    { key: "fetch_records", icon: FileDown, title: t("abdmActionFetchRecordsTitle"), desc: t("abdmActionFetchRecordsDesc"), color: "bg-red-100 text-red-700" },
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <h2 className="mt-3 text-2xl sm:text-3xl font-bold text-red-900">{t("abdmTitle")}</h2>
          ...
          return (
            <Card
              ...
              className={[
                ...
                done ? "border-red-300 bg-red-50/30" : "border-red-100",
              ].join(" ")}
            >
              <CardContent className="p-5">
                <div className="flex items-start gap-3">
                  <div className={[
                    ...
                    done ? "bg-red-600 text-white" : color,
                  ].join(" ")}>
      ...
      <Card className="border-red-100 shadow-sm">
        <CardHeader>
          <CardTitle className="text-base font-semibold text-red-900 flex items-center gap-2">
            <Activity className="size-4 text-red-600" /> {t("abdmActivityTitle")}
          </CardTitle>
          ...
          {abdmRecords.length === 0 ? (
            ...
            <div className="space-y-2 max-h-80 overflow-y-auto scrollbar-thin">
              {abdmRecords.map((r) => (
                <div
                  key={r.id}
                  className="rounded-lg border border-red-100 bg-white px-3 py-2.5 flex items-start gap-3"
                >
                  <div className={[
                    "size-7 rounded-full flex items-center justify-center shrink-0",
                    r.status === "success" ? "bg-red-100 text-red-700" : "bg-red-100 text-red-700",
                  ].join(" ")}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-red-900 mt-0.5">{r.message}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="rounded-xl bg-red-50 border border-red-200 p-4 flex items-start gap-3 text-sm text-red-800">
        <ShieldCheck className="size-5 shrink-0 text-red-600" />
        <div>
          <p className="font-semibold">{t("abdmPrivacyTitle")}</p>
          <p className="text-red-700/80 mt-1">{t("abdmPrivacyBody")}</p>
        </div>
      </div>
